...
#Cycle through all sheets, extract sheet name, and assign each sheet (Town) its own data frame
...
for (i in 1:length(dfs)) {
  ...
  current_geog <- current_geog[grepl("X60-X84", current_geog$`Cause of Death`),]
...
write.table(
  all_geogs_fips,
  file.path(getwd(), "data", "suicide_2008-2012.csv"),
  sep = ",",
  na = "-6666",
  row.names = F
)
